...
function LogoIcon({ type, accent }) {
  const dark = "#0B1F2A"
  const teal = "#46C7D8"
  const soft = accent ? "rgba(70,199,216,0.18)" : "rgba(255,255,255,0.68)"
  const line = accent ? "rgba(70,199,216,0.42)" : "rgba(11,31,42,0.12)"
...
function ModuleLogo({ item }) {
  return (
    <div className="mx-5 flex min-w-[178px] items-center gap-3 opacity-70 transition-opacity duration-300 hover:opacity-100 md:mx-6 md:min-w-[196px]">
      <LogoIcon type={item.type} accent={item.accent} />

      <div className="min-w-0">
        <p
          className={[
            "font-en text-[14px] font-semibold leading-none tracking-[-0.035em]",
            item.accent ? "text-[#0B6070]" : "text-nexus-dark/70",
          ].join(" ")}
        >
          {item.name}
        </p>

        <p className="mt-[6px] font-jp text-[10px] font-medium tracking-[0.08em] text-nexus-muted/58">
          {item.sub}
        </p>
      </div>
    </div>
...
export default function FlowBand() {
  ...
  return (
    <section
      className="relative z-20 overflow-hidden border-y border-nexus-line/70 bg-white/48 py-5 backdrop-blur-md md:py-6"
      aria-label="対応領域"
    >
      <style>
        {`
          @keyframes nexusFlowBand {
            from {
          ...
          .nexus-flow-band-track {
            animation: nexusFlowBand 76s linear infinite;
          }

          .nexus-flow-band:hover .nexus-flow-band-track {
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, rgba(255,255,255,0.9), rgba(255,255,255,0.28) 46%, rgba(255,255,255,0.9)), radial-gradient(circle at 18% 50%, rgba(70,199,216,0.09), transparent 30%), radial-gradient(circle at 78% 50%, rgba(122,220,191,0.08), transparent 32%)",
        }}
      />

      {/* edge fade */}
      <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-white via-white/82 to-transparent md:w-52" />
      <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-white via-white/82 to-transparent md:w-52" />

      {/* center line */}
      <div className="pointer-events-none absolute left-0 right-0 top-1/2 h-px bg-gradient-to-r from-transparent via-nexus-line/70 to-transparent" />



      <div className="nexus-flow-band relative flex overflow-hidden">
        <div
          className="nexus-flow-band-track flex w-max items-center will-change-transform"
          aria-hidden="true"
        >
          {loop.map((item, index) => (
